/*-----------------------------------------------
	...
	- 로그인 액션 처리 전용 클래스
	- 로그인 액션 처리 이후
	  『employeelist.action』/ 『emplist.action』을
	   요청 할 수 있도록 처리 
...
public class LoginController implements Controller
{
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		// 데이터 수신(LoginForm.jsp 로 부터.. id, pw , admin 수신)
		...
		if(name == null) 
		{
			// 로그인 실패 → 로그인 폼을 다시 요청 할 수 있도록 안내
		...
		}else 
		{
			...
			{
				// → emplist.action 페이지를 요청 할 수 있도록 안내
				mav.setViewName("redirect:emplist.action");
				
			}else			// 관리자로 로그인 성공한 상황.. 
			{
				// → admin 속성을 세션에 추가
				// 	  employeelist.action 페이지를 요청 할 수 있도록 안내
